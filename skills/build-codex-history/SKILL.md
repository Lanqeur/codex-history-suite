...
4. Report source/change counts, disk volume, summarization mode, estimated tokens, and estimated CNY cost. Never start paid work without a user-approved `--max-cost-cny` value.
5. Run `build --max-cost-cny N --json` or `update --max-cost-cny N --json`. Keep promotion enabled unless the user explicitly requests a staging-only build.
6. Confirm every state-machine stage completed and the build audit passed. A failed build must leave the prior `active.json` untouched.
7. After the first incremental update, after pipeline changes, or when correctness is disputed, run `audit --equivalence --json`. Treat any table digest difference as a release failure.
