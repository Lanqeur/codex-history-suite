...
Delta v2 is the default. Its `delta.json` is a bounded manifest containing the target source/file inventories, stable lineage, exact base/target generations, capability versions, and hashes. Large artifact mappings are stored in `data/metadata/artifact-metadata.jsonl.gz`; verification hashes the compressed file and then validates every decompressed row, table count, and stream trailer without loading the corpus into memory. Only transcript chunks and artifact CAS objects absent from the base are packaged.

V2 also includes an affected-thread/scope authority patch and semantic-index file differences. This transfers the producer's completed ledger, Overview, claims, evidence links, relations, and vectors instead of asking the receiver to regenerate them. A successful receiver reports `apply_mode: precomputed` and zero API tokens. Deletes and any failed table-count, source-generation, foreign-key, SQLite, artifact-digest, or stream validation trigger `rebuild_fallback`; the result retains `fast_apply_error` and the canonical pipeline remains the correctness backstop. Format v1 remains available with `export-delta --format 1` for bounded compatibility cases. Readers reject any legacy `delta.json` above the 16 MiB safety limit before parsing; rebuild such transfers as v2 instead of risking unbounded memory use.

`apply-delta` is restricted to profile-managed imported sources. It requires the receiver's active source generation to equal either the declared base or target generation, stages immutable blobs, then creates one copy-on-write candidate. Precomputed authority, artifact rows, and semantic files become visible together only after quick SQLite, foreign-key, table-count, source-generation, and digest checks pass. Artifact-only deltas remain zero-model operations. Idempotence requires both source generation and artifact inventory/metadata to match; an equal source generation alone does not skip pending artifact mappings. On fallback source reconstruction failure it restores source files. A skipped delta, unrelated library, divergent local generation, unsafe archive path, or content mismatch is a hard error. `--artifacts none` remains an intentional query-only transport and does not claim artifact convergence.

Model-cache deltas remain enabled as a fallback aid, and cache keys exclude device-local build IDs. In the normal precomputed path they are not needed to reconstruct summaries because producer-computed knowledge and semantic state are already present. Omitting caches can still cause paid work if the receiver must use `rebuild_fallback`; review the target plan and pass an explicit `--max-cost-cny` when fallback configuration can call a model. Chroma changes are staged copy-on-write and swapped with the SQLite authority.
